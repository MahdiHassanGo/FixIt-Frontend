...
export function ServiceForm({ categories, service }: { categories: Category[]; service?: Service }) {
  ...
  return (
    <form action={formAction} className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-2 sm:col-span-2"><Label htmlFor={`category-${service?.id || "new"}`}>Category</Label><select id={`category-${service?.id || "new"}`} name="categoryId" defaultValue={service?.categoryId || ""} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="">Select category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><FieldError messages={state?.fieldErrors?.categoryId} /></div>
